...
DD_1 <- lm(formula = rprice ~ nearinc + as.factor(year), data=data)
se_DD_cap1 <- starprep(DD_1, stat = c("std.error"), se_type = "HC2", alpha = 0.05) 

DD_2 <- lm(formula = rprice ~ nearinc + as.factor(year), data=data)
se_DD_cap2 <- starprep(DD_2, stat = c("std.error"), se_type = "CR2", clusters=RPS$state_name, alpha = 0.05) 

se_models <- list(se_DD_1[[1]], se_DD_2[[1]])
stargazer(DD_1, DD_2, se = se_models, keep=c("nearinc"), type="text")
```

**The difference-in-differences model predicts a coefficient of -25,052 for `nearinc`. This corresponds to about a $25,000 lower price for houses near the incinerator in 1981 compared to  what those same houses _would_ have been worth in 1981 without the incinerator (thus, it is the causal effect of the incinerator, not biased by the systemic differences in housing values seen in 1978). This is done using the parallel trends assumption.**

### (e) Report the 95% confidence interval for the estimate of the causal effect on the incinerator in (d).

```{r}
DDse = lmtest::coeftest(DDmodel, vcov = sandwich::vcovHC(DDmodel, type = "HC2"))[, "Std. Error"]

DDCI = list()

DDCI[1] = -25052 - (DDse * 1.96)
DDCI[2] = -25052 + (DDse * 1.96)
```

**The 95% CI for the estimate in (d) is .**
